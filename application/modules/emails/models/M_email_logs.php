...
class M_email_logs extends CI_Model {
    ...
    private function generate_where($args) {
        if (isset($args['to'])) {
            $this->db->or_like($this->_table_name . ".to", $args['to']);
        }
        
        if (isset($args['email'])) {
            $this->db->or_like($this->_table_name . ".email", $args['email']);
        }
        
        if (isset($args['subject'])) {
            $this->db->or_like($this->_table_name . ".subject", $args['subject']);
        }
        
        if (isset($args['content'])) {
            $this->db->or_like($this->_table_name . ".content", $args['content']);
        }
